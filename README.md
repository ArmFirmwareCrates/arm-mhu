# Arm Message Handling Unit (MHU) driver

Driver implementation for the Arm
[Message Handling Unit](https://developer.arm.com/documentation/aes0072/ab/) (ARM-AES-0072 A.b).

It provides a self-contained driver for each MHU component, channel, or page (control, doorbell,
FIFO, fast channel, and security control), as well as a top-level driver that manages the MHU as a
whole. With the top-level driver, users can borrow, take, or split the listed components into
individual drivers as needed.

## Implemented features

* MHU v3 support
* Separate Postbox and Mailbox drivers
* Doorbell Channel support
* FIFO Channel support
* Fast Channel support
* TrustZone and Realm Management Extensions
* Unit tests

## License

The project is MIT and Apache-2.0 dual licensed, see `LICENSE-Apache-2.0` and `LICENSE-MIT`.

## Maintainers

arm-mhu is a trustedfirmware.org maintained project. All contributions are ultimately merged by the maintainers
listed below.

* Bálint Dobszay <balint.dobszay@arm.com>
  [balint-dobszay-arm](https://github.com/balint-dobszay-arm)
* Imre Kis <imre.kis@arm.com>
  [imre-kis-arm](https://github.com/imre-kis-arm)
* Sandrine Afsa <sandrine.afsa@arm.com>
  [sandrine-bailleux-arm](https://github.com/sandrine-bailleux-arm)

## Contributing

Please follow the directions of the [Trusted Firmware Processes](https://trusted-firmware-docs.readthedocs.io/en/latest/generic_processes/index.html)

Contributions are handled through [review.trustedfirmware.org](https://review.trustedfirmware.org/q/project:arm-firmware-crates/arm-mhu).

## Arm trademark notice

Arm is a registered trademark of Arm Limited (or its subsidiaries or affiliates).

This project uses some of the Arm product, service or technology trademarks, as listed in the
[Trademark List][1], in accordance with the [Arm Trademark Use Guidelines][2].

Subsequent uses of these trademarks throughout this repository do not need to be prefixed with the
Arm word trademark.

[1]: https://www.arm.com/company/policies/trademarks/arm-trademark-list
[2]: https://www.arm.com/company/policies/trademarks/guidelines-trademarks

--------------

*Copyright The arm-mhu Contributors.*
